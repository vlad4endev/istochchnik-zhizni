...
export function notesFromItem(raw: MusicianNotesV1 | undefined | null): MusicianNotesV1 {
  ...
  return {
    v: 1,
    lineComments:
      raw.lineComments && typeof raw.lineComments === 'object'
        ? { ...raw.lineComments }
        : undefined,
    blockComments: Array.isArray(raw.blockComments)
      ? raw.blockComments.map((b) => ({ ...b }))
      : undefined,
  };
}

export function musicianNotesCount(raw: MusicianNotesV1 | undefined | null): number {
  const n = notesFromItem(raw);
  const lines = Object.values(n.lineComments ?? {}).filter((t) => t.trim()).length;
  const blocks = (n.blockComments ?? []).filter((b) => b.text?.trim()).length;
  return lines + blocks;
}
