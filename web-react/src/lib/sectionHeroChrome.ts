...
export const sectionHeroStickyClass =
  'flex-shrink-0 bg-[var(--surface)]/95 shadow-[0_4px_16px_rgba(0,0,0,0.02)] backdrop-blur-md supports-[backdrop-filter]:bg-[var(--surface)]/80 px-3 pt-[max(0.5rem,env(safe-area-inset-top,0px))] pb-2 sm:px-4 sm:pt-[max(0.75rem,env(safe-area-inset-top,0px))] shell:px-6 md:px-6 lg:px-8 xl:px-10';

/** То же без горизонтального px — внутри уже ограниченной колонки (дашборд) */
export const sectionHeroStickyClassNested =
  'flex-shrink-0 bg-[var(--surface)]/95 shadow-[0_4px_16px_rgba(0,0,0,0.02)] backdrop-blur-md supports-[backdrop-filter]:bg-[var(--surface)]/80 pt-[max(0.5rem,env(safe-area-inset-top,0px))] pb-2 sm:pt-[max(0.75rem,env(safe-area-inset-top,0px))]';
