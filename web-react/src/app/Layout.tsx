...
function navClassName(isActive: boolean, compact = false): string {
  const base = compact
    ? 'group relative flex min-w-0 flex-1 basis-0 flex-col items-center justify-center overflow-visible rounded-xl px-0.5 py-1 transition-[transform,color] duration-200 ease-out tap-highlight-transparent touch-manipulation outline-none motion-reduce:transition-none motion-reduce:active:scale-100 focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent active:scale-[0.96]'
...
function MobileNavOverflow({
  ...
  return (
    <div className="relative flex min-w-0 flex-1 basis-0 flex-col items-stretch">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <span
          className={[
            ...
            isMoreTabActive ? 'bg-primary/[0.12]' : 'group-active:bg-black/[0.05] dark:group-active:bg-white/[0.08]',
          ].join(' ')}
        >
          <LuEllipsis className={navIconClass(isMoreTabActive, true)} strokeWidth={2} aria-hidden />
        </span>
        <span className="mt-0.5 w-full min-w-0 px-px text-center text-[10px] font-semibold leading-[1.1] tracking-tight text-inherit line-clamp-1">
          Ещё
        </span>
      </button>
      {open ? (
...
export function Layout() {
  ...
  return (
    ...
    <div className="flex min-h-0 w-full max-w-full flex-1 flex-col bg-[var(--surface)] text-[var(--text)]">
      ...
      >
        <div className="flex w-full min-h-[var(--app-bottom-nav-bar-height)] min-w-0 items-stretch justify-between gap-0 px-1 pb-0 pt-1 sm:px-2">
              ...
              >
                {({ isActive }) => (
                  <>
                    <span
                      className={[
                        'relative z-10 grid h-8 w-8 place-items-center overflow-visible rounded-full transition-colors duration-200',
                        isActive ? 'bg-primary/[0.12]' : 'group-active:bg-black/[0.05] dark:group-active:bg-white/[0.08]',
                      ].join(' ')}
                    >
                      <Icon className={navIconClass(isActive, true)} strokeWidth={2} aria-hidden />
                      {item.to === '/messenger' && activityBadgeTotal > 0 ? (
                        <span className="absolute -right-1.5 -top-0.5 z-[5] inline-flex min-h-[16px] min-w-[16px] items-center justify-center rounded-full bg-rose-500 px-1 text-[9px] font-extrabold leading-none text-white shadow-sm ring-2 ring-white dark:ring-[color-mix(in_srgb,var(--surface-elevated)_92%,transparent)]">
                          {formatNavBadgeCount(activityBadgeTotal)}
                        </span>
                      ) : null}
                    </span>
