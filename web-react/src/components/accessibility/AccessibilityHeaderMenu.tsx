...
export function AccessibilityHeaderMenu({ tone = 'on-gradient', triggerClassName }: Props) {
  ...
  const panel = open ? (
    <div
      ...
      className={
        layout === 'sheet'
          ? `${panelBaseClass} fixed inset-x-3 top-[max(0.5rem,env(safe-area-inset-top,0px))] mx-auto w-auto max-w-lg max-h-[min(88dvh,calc(100dvh-env(safe-area-inset-top,0px)-var(--app-bottom-nav-total-height)))]`
