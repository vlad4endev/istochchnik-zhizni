...
import {
  ...
  fetchMusicMembers,
  fetchMusicRoles,
  removeMusicAssignment,
  reorderMusicRoles,
  updateMusicRole,
} from '../api';
import { EventModal } from '../components/EventModal';
import { MusicSchedulePlanMobileCards } from '../components/MusicSchedulePlanMobileCards';
...
import type { MusicEvent, MusicScheduleViewMode } from '../types';
import {
  ASSEMBLY_PAGE_SIZE,
...
} from '../utils/musicScheduleWeekView';
...
function assignmentCoverage(ev: MusicEvent, totalRoles: number): { filled: number; total: number } {
...
export function MusicSchedulePage() {
  ...
  const meQ = useMe();
  const canManage =
    meQ.data != null ? canManageMusicSchedule(role, meQ.data.ministry_role, rolesAuth) : false;
  ...
  const eventsQ = useQuery({
    queryKey: ['music-schedule', 'events', ymd(rangeFrom), ymd(rangeTo)],
    queryFn: () => fetchMusicEvents(rangeFrom, rangeTo),
  });

  const rolesQ = useQuery({
    queryKey: ['music-schedule', 'roles'],
    queryFn: fetchMusicRoles,
  });

  const membersQ = useQuery({
    queryKey: ['music-schedule', 'members'],
    queryFn: () => fetchMusicMembers(),
  });
  ...
  const assignMut = useMutation({
    mutationFn: async ({ eventId, memberId: mid, roleId }: { eventId: number; memberId: number; roleId: number }) => {
      await assignMusicMember(eventId, mid, roleId);
    },
    onSuccess: invalidate,
  });

  const removeMut = useMutation({
    mutationFn: removeMusicAssignment,
    onSuccess: invalidate,
  });
      ...
      <div className="mx-auto w-full max-w-6xl space-y-3 sm:space-y-4">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[13px] font-semibold leading-snug text-stone-500 sm:text-sm">
              Музыкальное служение — службы и назначения команды
              {upcomingCount > 0 ? (
        ...
        <RolesSettingsModal
          ...
          onCreate={async (input) => {
            await createMusicRole(input);
            invalidate();
          }}
          onUpdate={async (id, input) => {
            await updateMusicRole(id, input);
            invalidate();
          }}
          onDelete={async (id) => {
            await deleteMusicRole(id);
            invalidate();
          }}
          onReorder={async (ids) => {
            await reorderMusicRoles(ids);
            invalidate();
          }}
        />
      </div>
