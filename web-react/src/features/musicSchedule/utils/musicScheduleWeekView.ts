...
export function sortMusicEventsChronologically(events: MusicEvent[]): MusicEvent[] {
  return [...events].sort(
    (a, b) =>
...
export function buildServiceEventColumns(events: MusicEvent[]): WeekEventColumn[] {
  return sortMusicEventsChronologically(events).map((ev) => ({
...
export function buildWeekEventColumns(
  eventsByDate: Map<string, MusicEvent[]>,
  daysWithEvents: Date[],
): WeekEventColumn[] {
