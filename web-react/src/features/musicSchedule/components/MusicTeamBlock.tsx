...
import { fetchMusicAssignmentsForPlan, fetchMusicEventByPlanId } from '../api';
import type { MusicAssignment } from '../types';
...
export function MusicTeamBlock({
  planId,
  canManage = false,
  compact = false,
  embedded = false,
  ...
  const eventQ = useQuery({
    queryKey: ['music-schedule', 'plan', planId],
    queryFn: () => fetchMusicEventByPlanId(planId!),
    enabled: planId != null && planId > 0,
  });

  const assignmentsQ = useQuery({
    queryKey: ['music-schedule', 'plan-assignments', planId],
    queryFn: () => fetchMusicAssignmentsForPlan(planId!),
    enabled: planId != null && planId > 0,
  });
  ...
  const heading = title ?? (embedded ? 'Музыкальная команда' : 'Музыкальная команда');
  ...
  const content = loading ? (
    ...
    <p className={embedded ? 'text-xs text-stone-500' : 'text-sm text-[var(--text-muted)]'}>
      ...
      {showManageActions ? (
        <>
          {' '}
          <Link to={`/music-schedule?planId=${planId}`} className="font-semibold text-primary">
            Назначить команду
          </Link>
        </>
      ) : null}
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        {showManageActions ? (
          <Link
            to={`/music-schedule?planId=${planId}`}
...
function AssignmentRow({ assignment, embedded = false }: { assignment: MusicAssignment; embedded?: boolean }) {
  if (embedded) {
    return (
