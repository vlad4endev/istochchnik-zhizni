...
import type { MusicEvent, MusicRole, MusicScheduleMember } from '../types';
...
export function EventModal({
  ...
  const [localEvent, setLocalEvent] = useState<MusicEvent | null>(null);
  ...
  useEffect(() => {
    ...
    setError(null);
    let cancelled = false;
    void fetchMusicEventByPlanId(planId)
      .then((ev) => {
  ...
  const assignmentsByRole = useMemo(() => {
    const map = new Map<number, MusicEvent['assignments'][number] | undefined>();
  ...
  async function refreshAfterMutation() {
    if (planId != null) {
      const ev = await fetchMusicEventByPlanId(planId);
      setLocalEvent(ev);
    }
    onSaved();
  }
  ...
  async function handleRemoveAssignment(assignmentId: number) {
    setBusy(true);
    try {
      await removeMusicAssignment(assignmentId);
  ...
  async function handleAssign(memberId: number, roleId: number) {
    ...
    await assignMusicMember(planId, memberId, roleId);
    await refreshAfterMutation();
    setPickerOpen(false);
  }
  ...
  async function handleStatus(assignmentId: number, status: 'confirmed' | 'declined') {
    setBusy(true);
    try {
      await updateMusicAssignmentStatus(assignmentId, status);
      await refreshAfterMutation();
    } catch (e) {
      setError(apiErrorMessage(e));
    } finally {
