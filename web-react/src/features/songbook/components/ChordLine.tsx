...
export function ChordLine({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-end gap-x-0 [row-gap:0]">
        {segments.map((seg, idx) => (
          <span key={idx} className="relative inline-block max-w-full align-bottom">
            {seg.chord ? (
              onChordClick ? (
                <button
                  type="button"
            ...
            <span className="inline-block whitespace-pre-wrap">{seg.text}</span>
          </span>
        ))}
      </div>
    </div>
