...
export function AccessRequestsSection() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <p className="text-sm text-[var(--text-secondary)]">Загрузка заявок…</p>
      ) : error ? (
        <p className="text-sm text-red-600">{apiErrorMessage(error, 'Не удалось загрузить заявки')}</p>
      ) : pending.length === 0 ? (
        <div className="flex min-h-[220px] items-center justify-center rounded-2xl border border-[#F0E9EA] bg-[var(--surface-elevated)] px-6 py-14 text-center">
          <div className="flex flex-col items-center">
            <LuMail className="h-10 w-10 text-[#7B2D3F]" aria-hidden />
            <p className="mt-4 text-xl font-medium text-[var(--text)]">Нет заявок</p>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">Здесь появятся заявки на регистрацию и сброс пароля</p>
          </div>
        </div>
      ) : filteredPending.length === 0 ? (
        <div className="flex min-h-[160px] items-center justify-center rounded-2xl border border-stone-200 bg-[var(--surface)] px-6 py-10 text-center">
          <p className="text-sm text-[var(--text-secondary)]">По выбранному фильтру заявок нет</p>
        </div>
      ) : (
        <ul className="space-y-2">
          {isFetching && !isLoading ? (
            <p className="text-xs text-[var(--text-muted)]">Обновление…</p>
          ) : null}
...
function AccessRequestRow(props: {
  ...
  return (
    <li className="rounded-[10px] border border-[#F0E9EA] bg-[var(--surface-elevated)] px-4 py-3.5">
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-[var(--text-secondary)]">{item.phone_number}</p>
          </div>
        </div>
          ...
          <span
            className={
              item.request_type === 'password_reset'
                ? 'inline-flex rounded-[10px] bg-orange-50 px-2 py-0.5 text-xs font-medium text-orange-700'
                : 'inline-flex rounded-[10px] bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700'
            }
          >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            ...
            disabled={busy}
            onClick={onApprove}
            className="rounded-md bg-[var(--primary)] px-3.5 py-1.5 text-sm font-medium text-white transition hover:opacity-95 disabled:opacity-50"
          >
            Принять
          </button>
          {item.request_type !== 'password_reset' ? (
          ...
          <button
            ...
            disabled={busy}
            onClick={onReject}
            className="rounded-md border border-stone-200 bg-transparent px-3.5 py-1.5 text-sm font-medium text-[var(--text-secondary)] transition hover:bg-[var(--surface)] disabled:opacity-50"
          >
            Отклонить
          </button>
        </div>
...
function FilterTab(props: { active: boolean; label: string; count: number; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex cursor-pointer items-center rounded-[20px] border px-[14px] py-1.5 text-sm transition ${
        active
    ...
    >
      {label}
      <span
        className={`ml-1 inline-flex h-[18px] min-w-[18px] items-center justify-center rounded-[9px] px-1 text-xs font-semibold ${
          active ? 'bg-white/25 text-white' : 'bg-[var(--surface)] text-[var(--text-secondary)]'
        }`}
      >
        {count}
      </span>
