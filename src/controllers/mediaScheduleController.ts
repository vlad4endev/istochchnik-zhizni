...
async function ensureManager(req: Request, res: Response): Promise<number | null> {
  ...
  if (!(await canManageMediaSchedule(req))) {
    res.status(403).json({ error: 'Недостаточно прав для управления расписанием медиа-служения' });
    return null;
  }
  return userId;
}
...
function handleServiceError(res: Response, err: unknown, context: string): void {
  ...
  console.error(`[media-schedule] ${context}:`, err);
...
export async function getEvents(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    void sendMediaScheduleReminders().catch((e) =>
      console.warn('[media-schedule] reminders tick failed', e),
    );
    res.json({ events });
  } catch (err) {
...
export async function updateAssignmentStatusHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (assignment.member_id !== userId && !(await canManageMediaSchedule(req))) {
