...
CREATE TABLE IF NOT EXISTS global_settings (
  ...
  telegram_service_plan_published_template TEXT,
  telegram_service_plan_published_button_text TEXT,
  telegram_service_plan_mailing_destinations JSONB,
  telegram_service_plan_published_destinations JSONB,
...
ALTER TABLE members ADD COLUMN IF NOT EXISTS telegram_username VARCHAR(64);
...
ALTER TABLE global_settings ADD COLUMN IF NOT EXISTS telegram_service_plan_published_template TEXT;
...
CREATE INDEX IF NOT EXISTS idx_push_subscriptions_last_used_at
  ON push_subscriptions (last_used_at DESC NULLS LAST);
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'message_payload_type') THEN
    ...
    EXCEPTION WHEN duplicate_object THEN
      NULL;
    END;
    -- video_note: только через pg_enum — иначе на части сборок PG дубль не ловится как duplicate_object
    IF NOT EXISTS (
      SELECT 1
      FROM pg_enum e
...
-- Watermark просмотра ленты (бейдж новых постов в меню).
...
INSERT INTO media_roles (name, color, sort_order)
VALUES
  ('Режиссёр',   '#7d3640', 1),
  ('Слова',      '#0891b2', 2),
  ('Оператор 1', '#059669', 3),
...
CREATE TABLE IF NOT EXISTS media_assignments (
  id            BIGSERIAL PRIMARY KEY,
  event_ref_id  BIGINT NOT NULL REFERENCES service_plans(id) ON DELETE CASCADE,
  member_id     BIGINT NOT NULL REFERENCES members(id) ON DELETE CASCADE,
  role_id       BIGINT NOT NULL REFERENCES media_roles(id) ON DELETE RESTRICT,
...
-- Android APK / app releases (download widget on home + admin «Релизы»)
