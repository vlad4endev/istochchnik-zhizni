...
export interface MediaAssignmentMember {
  id: number;
  name: string;
...
export interface MediaAssignment {
  ...
  notes: string | null;
  member: MediaAssignmentMember;
  role: MediaRole;
}

export interface EventWithAssignments extends MediaEvent {
...
const MEDIA_MEMBER_DIRECTION_PATTERNS = [
  '%медиа%',
  '%media%',
  '%оператор%',
  '%звук%',
...
function mapRoleRow(row: Record<string, unknown>): MediaRole {
  return {
    id: Number(row.id),
...
function mapEventRow(row: Record<string, unknown>): MediaEvent {
...
async function fetchAssignmentsForPlanIds(planIds: number[]): Promise<Map<number, MediaAssignment[]>> {
  const out = new Map<number, MediaAssignment[]>();
...
async function attachAssignments(events: MediaEvent[]): Promise<EventWithAssignments[]> {
...
export async function getAssignmentsForPlan(planId: number): Promise<MediaAssignment[]> {
...
export async function assignMember(
  ...
  const roleExists = await query(`SELECT * FROM media_roles WHERE id = $1 AND is_active = TRUE`, [roleId]);
  ...
  const existingSameRole = await query(
    `SELECT id FROM media_assignments WHERE event_ref_id = $1 AND role_id = $2 LIMIT 1`,
    [eventRefId, roleId],
  );
  ...
  try {
    const result = await query(
      `INSERT INTO media_assignments (event_ref_id, member_id, role_id, status, created_by)
    ...
    ).catch((e) => console.warn('[media-schedule] assignment push failed', e));
...
export async function removeAssignment(assignmentId: number): Promise<void> {
  ...
  const result = await query(`DELETE FROM media_assignments WHERE id = $1`, [assignmentId]);
  ...
  if (planCtx) {
    ...
    ).catch((e) => console.warn('[media-schedule] cancellation push failed', e));
  }
}
...
export async function getMemberSchedule(
  ...
  const result = await query(
    `${PLAN_SELECT}
     JOIN media_assignments a ON a.event_ref_id = p.id
     WHERE a.member_id = $1
       AND p.service_date >= $2::date
...
export async function sendMediaScheduleReminders(): Promise<number> {
  const result = await query(
    `SELECT
  ...
  for (const row of result.rows as Array<Record<string, unknown>>) {
    ...
    try {
      ...
      await query(
        `UPDATE media_assignments SET reminder_sent_at = NOW() WHERE id = $1`,
        [assignmentId],
      );
      sent += 1;
    } catch (e) {
      console.warn(`[media-schedule] reminder push failed for assignment ${assignmentId}`, e);
    }
  }
  return sent;
}

export async function getRoles(): Promise<MediaRole[]> {
  const result = await query(
    `SELECT * FROM media_roles WHERE is_active = TRUE ORDER BY sort_order ASC, id ASC`,
  );
  return (result.rows as Record<string, unknown>[]).map(mapRoleRow);
}

export async function getAllRolesIncludingInactive(): Promise<MediaRole[]> {
  const result = await query(`SELECT * FROM media_roles ORDER BY sort_order ASC, id ASC`);
  return (result.rows as Record<string, unknown>[]).map(mapRoleRow);
}

export async function createRole(input: CreateRoleInput): Promise<MediaRole> {
  const name = input.name.trim();
  if (!name) throw new Error('invalid_input');

  const maxOrder = await query(`SELECT COALESCE(MAX(sort_order), 0)::int AS m FROM media_roles`);
  const nextOrder =
...
export async function updateRole(id: number, input: UpdateRoleInput): Promise<MediaRole> {
  const fields: string[] = [];
  ...
  if (fields.length === 0) {
    const row = await query(`SELECT * FROM media_roles WHERE id = $1`, [id]);
  ...
  values.push(id);
  const result = await query(
    `UPDATE media_roles SET ${fields.join(', ')} WHERE id = $${idx} RETURNING *`,
    values,
  );
...
export async function deleteRole(id: number): Promise<void> {
  const used = await query(`SELECT 1 FROM media_assignments WHERE role_id = $1 LIMIT 1`, [id]);
  if (used.rows[0]) {
    throw new Error('role_in_use');
  }
  const result = await query(`DELETE FROM media_roles WHERE id = $1`, [id]);
  if ((result.rowCount ?? 0) === 0) {
    throw new Error('not_found');
  }
}
...
export async function reorderRoles(ids: number[]): Promise<void> {
  ...
  for (let i = 0; i < ids.length; i += 1) {
    ...
    if (id == null) continue;
    await query(`UPDATE media_roles SET sort_order = $2 WHERE id = $1`, [id, i + 1]);
  }
}
